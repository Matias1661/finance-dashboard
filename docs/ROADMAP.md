...
- [ ] Migración Relay → GitHub Actions, flujos pendientes de implementar (ver `PROJECT_MEMORY.md`, sección "Migración en curso", y `DECISIONS.md` 2026-07-17 para prompts ya probados):
  ...
  - [x] Talho Argentino / Gastos del local (paso 4.5 del plan): `scripts/process_gastos_talho.py` implementado (comprobante en Drive → Notion, deja "Pagado por" vacío). El sub-flujo de edición manual en Notion no necesita código nuevo, ya lo cubre el cron diario de `sync_sociedad_data.py`. Pendiente: compartir la carpeta Drive con la cuenta de servicio (acción manual del usuario) y validar con un comprobante real (ver `DECISIONS.md` 2026-07-22).
