...
| **Nóminas** | ⚠️ Implementado (`scripts/process_nominas.py`, 22/07/2026) pero **NO validado de forma aislada**: al probar con la nómina real de julio 2026 (31/07/2026) se confirmó que los dos flujos de Relay para Nóminas (email de Beatriz → Drive, y Drive → Notion) **siguen activos** en Relay.app — nunca fueron desactivados pese a que esta sección asumía que Matías subía el PDF a mano en su reemplazo. Resultado: Relay subió y procesó el PDF automáticamente, y la subida manual de Matías generó una segunda página duplicada en Notion vía GitHub Actions. Ver `docs/DECISIONS.md` 2026-07-31. **Acción pendiente del usuario:** desactivar ambos flujos de Nóminas en Relay.app, luego repetir la validación solo con GitHub Actions antes de marcar la fila 4 del plan como Completado. Prompt de extracción "Extraer datos de nómina" en la DB "Prompts" (empresa normalizada a "LUZUTANIA GROUP" — en la corrida del 31/07 se extrajo como "LUZUTANIAES GROUP SLU", inconsistencia a revisar). `processed_nominas.json` sembrado con los 38 archivos ya existentes en la carpeta. Chequeo de duplicados por Fecha de pago en Notion (no llegó a probarse de forma aislada por la interferencia de Relay). Nota técnica: las nóminas del empleador actual (LUZUTANIA GROUP) son PDFs escaneados/imagen, a diferencia de las de BETWEEN TECHNOLOGY (texto seleccionable); el script manda el PDF completo a Claude como documento en ambos casos. |
