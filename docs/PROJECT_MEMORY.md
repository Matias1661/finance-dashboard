...
**Pendiente general:** escribir el script que falta (MyInvestor — reusar la autenticación Gmail OAuth ya resuelta para Peerberry), validarlo con datos reales comparando los agregados mensuales de `finance_data.json` contra una suma manual de las filas fuente (no alcanza con el log de "creado/duplicado" del script — lección del incidente de Peerberry, ver `DECISIONS.md` 2026-07-17), y seguir con los pasos 5-6 (validación en paralelo, apagado completo de Relay). Nóminas (fila 4) y Talho Argentino (paso 4.5) ya están completados y validados.
