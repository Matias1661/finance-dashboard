...
| **Nóminas** | ✅ Implementado (`scripts/process_nominas.py`, 22/07/2026), mismo patrón que `process_bank_statements.py`. Matías sube el PDF a mano a la carpeta Drive "Nominas" (ID `1pt9WGS3nzR8t1NIS5egrL0OMm2QGUKGj`) en vez de que el flujo lea el email de Beatriz automáticamente. Prompt de extracción "Extraer datos de nómina" nuevo en la DB "Prompts" (empresa normalizada a "LUZUTANIA GROUP", confirmado por el usuario). `processed_nominas.json` sembrado con los 38 archivos ya existentes en la carpeta (incluye una subida duplicada del mismo PDF con distinto file ID, detectada al revisar). Chequeo de duplicados por Fecha de pago agregado en Notion como defensa extra. Nota técnica: las nóminas del empleador actual (LUZUTANIA GROUP) son PDFs escaneados/imagen, a diferencia de las de BETWEEN TECHNOLOGY (texto seleccionable); el script manda el PDF completo a Claude como documento en ambos casos. **Pendiente de validar con la próxima nómina real** (julio 2026) antes de marcar la fila 4 del plan como Completado. |
| **Talho Argentino / Gastos del local (paso 4.5)** | ✅ Implementado y validado con datos reales (22/07/2026): 33 comprobantes de la carpeta Drive procesados, 29 duplicados detectados bien, 1 creado bien, 3 cargados a mano tras revision (fecha no legible en la foto). Bug corregido: un comprobante sin fecha extraible frenaba todo el script (ahora se maneja por archivo sin frenar el batch). Carpeta Drive confirmada compartida con la cuenta de servicio. El otro sub-flujo (edición manual en Notion con "Pagado por") no necesitó reemplazo: ya lo cubre el cron diario existente de `sync_sociedad_data.py`. Pendiente: decidir apagado de los 2 flujos de Relay para este paso. |
...
- El prompt de categorización de Movimientos se lee en vivo de la DB Notion "Prompts" en cada corrida (no está embebido) — ya replicado igual en `process_bank_statements.py`.
