...
| **Talho Argentino / Gastos del local (paso 4.5)** | ✅ Implementado (`scripts/process_gastos_talho.py` + paso en `sync-sociedad-data.yml`, 22/07/2026). Cubre el sub-flujo de comprobante en Drive (imagen o PDF → Claude extrae Concepto/Costo/Fecha → crea página en "Gastos del local" sin completar "Pagado por"). El otro sub-flujo (edición manual en Notion con "Pagado por") no necesitó reemplazo: ya lo cubre el cron diario existente de `sync_sociedad_data.py`. **Pendiente de acción manual del usuario:** compartir la carpeta Drive "Gastos Talho Argentino" (`1rlmEnN35OFcVmpWHylY9G3-qsK14-s0s`) con el email de la cuenta de servicio de `GOOGLE_SERVICE_ACCOUNT` — Matías no recordaba haberla compartido. Pendiente de validar con un comprobante real antes de marcar la fila 4.5 del plan como Completado. |
